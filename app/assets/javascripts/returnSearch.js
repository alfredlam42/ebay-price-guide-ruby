$(document).ready(function(){
  $('#search form').on('submit', function(event){
    ...
    //the following three variables are used for the template
    var searchParam = $('#item').val();
    var retailPrice = $('#retailPrice').val();
    var status = $('#completed')[0].checked ? 'Sold' : 'On Sale';
    ...
    .done(function(response){
      ...
      var templateData = createTemplate(searchParam, status, imageURL, calculations, profits)
...
function createTemplate(searchParam, status, imageURL, calculations, profits){
  var templateData = {
    name: searchParam,
    status: status,
    image: imageURL,
...
function trackItem(jsonData, searchParams){
  $('#track').on('submit', function(event){
    event.preventDefault();

    $.ajax({
      method: 'POST',
      data: {result: jsonData, searchParams: searchParams},
      url: '/result'
    })
